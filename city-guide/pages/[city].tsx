...
export async function getStaticPaths(): Promise<GetStaticPathsResult> {
  return {
    paths: getCities().map((city) => ({ params: { city: city.abbr } })),
    fallback: false,
  };
}
  ...
  return (
    <>
      <Head>
        ...
        />
        <link
          href="https://fonts.googleapis.com/css2?family=EB+Garamond:wght@400;500;700&family=Public+Sans:wght@400;500;700&display=swap"
          rel="stylesheet"
        />
        <meta
          name="viewport"
      ...
      <header className={styles.header}>
        {props.city.imgPosition === "none" ? null : (
          <img
            src={`${imgPrefix}/${props.city.abbr}.jpeg`}
            width="100%"
            height="100%"
            style={{ objectPosition: props.city.imgPosition }}
          />
        )}
        ...
        {Array.from(groups.entries()).map(([group, places], i) => {
          return (
            <div className={styles.list} key={i}>
              ...
              {props.city.categories && props.city.categories[group] ? (
                <p className={styles.listDescription}>
                  {props.city.categories[group].description}
                </p>
              ) : null}
              {places
                .sort(
                  (a, b) =>
                    Ratings.indexOf(a.rating) - Ratings.indexOf(b.rating)
                )
                .map((p, i) => {
                  return (
                    <div className={styles.place} key={i}>
                      <h3
                        style={{
                          textDecoration: p.closed ? "line-through" : undefined,
                        }}
                      >
                      ...
                      <div>
                        {p.closed ? (
                          <>
                            <p>{p.closed}</p>
                            <details className={styles.closedDetails}>
                              <summary>see previous notes</summary>
